    ...
    int hop_length)
{
    ...
    size_t num_frames = 1 + (samples.size() - frame_length) / hop_length;
    ...
    for (size_t i = 0; i < num_frames; ++i) {
        ...
        size_t end = std::min(start + frame_length, samples.size());
    ...
    int hop_length)
{
    ...
    for (size_t i = 0; i < num_frames; ++i) {
        ...
        // Normalize to [0, 1]
        zcr[i] = static_cast<float>(crossings) / (end - start - 1);
    }
