...
TEST_CASE("FFT cross-correlation finds correct lag for shifted signal", "[correlation][fft]") {
    ...
    for (size_t i = 0; i < reference.size(); ++i) {
        reference[i] = 0.1f + std::sin(2.0f * M_PI * 7.0f * i / 1000.0f);
    }
...
TEST_CASE("FFT cross-correlation with noise still locks on target", "[correlation][fft]") {
    ...
    for (size_t i = 0; i < reference.size(); ++i) {
        reference[i] = std::sin(2.0f * M_PI * 13.0f * i / 200.0f)
                     + 0.5f * std::sin(2.0f * M_PI * 3.0f * i / 200.0f);
    }
    ...
    for (size_t i = 0; i < target.size(); ++i) {
        ...
        float r = (static_cast<int32_t>(seed) / 2.0e9f);  // roughly -1..1
        target[i] = 0.05f * r;
    }
    for (size_t i = 0; i < reference.size(); ++i) {
        target[true_lag + i] += reference[i];
...
TEST_CASE("FFT correlation agrees with naive implementation", "[correlation][fft]") {
    ...
    for (size_t i = 0; i < reference.size(); ++i) {
        reference[i] = std::sin(2.0f * M_PI * 5.0f * i / 500.0f)
                     + 0.25f * std::cos(2.0f * M_PI * 17.0f * i / 500.0f);
    }
