...
- **Cured by.** **Two cure axes, three items.** The "binary returns non-zero" claim has two distinct failure components:
  - **Binary-exit-code axis (Catch2 semantic).** Cured by **C-4** (BACKLOG.md, Tier 5 — Algorithmic correctness). C-4 adds a passing TEST_CASE (`"Reference mode: native-rate boundary is rounded not truncated"`) to `tests/test_reference_mode.cpp` to verify its rate-conversion rounding helper. Catch2 returns 0 when at least one case passes alongside SKIPs, so the binary's exit code flips Failed→Passed at C-4's merge — independently of whether the SKIP cluster has been resolved. Empirically verified post-C-4 audit-1: `test_reference_mode` reports `1 passed | 2 skipped, exit=0`. The cure mechanism is structural (TEST_CASE count > 0 with at least one pass), not semantic (the SKIPs themselves still skip).
  - **SKIP-cluster axis (placeholders becoming real assertions).** Cured by two separate items, neither in PR #23's scope:
    - **M-REF-ALIGN-UNIT** (BACKLOG.md, Tier 5 — Algorithmic correctness) un-SKIPs `tests/test_reference_mode.cpp:14` (per-track-alignment placeholder) and replaces the SKIP with real assertions against the FIXTURE-REF v1 manifest at `tests/fixtures/ref_v1/manifest.txt`.
    - **Mi-17** (BACKLOG.md, Tier 9 — extended) un-SKIPs `tests/test_reference_mode.cpp:20` (natural-filename-sort placeholder), alongside its original mandate to harden `natural_less` against `std::stoll` overflow.

  The two axes are orthogonal cures of the same entry along different surfaces. This entry is fully resolved only when all three items have landed; the binary's exit-code surface resolves at C-4 merge, the SKIP-cluster surface resolves at M-REF-ALIGN-UNIT + Mi-17 paired merge.
- **Why this entry was misattributed previously (two catches).**
  1. **First catch (pre-staged version, PR #23 era).** The doc claimed PR #23 cures `test_reference_mode`. That was an audit-pass-required failure: PR #23's BACKLOG-stated scope is the three `[integration][reference]` cases in `test_integration.cpp`, which were SKIP'd before #23 and PASS after #23 (41 assertions in 3 test cases). Neither the doc nor the BACKLOG ever had PR #23 covering the standalone binary's two SKIPs. Recorded in `feedback_pre_staged_docs_need_audit.md`.
  2. **Second catch (C-4 dispatch, 2026-05-04).** The post-first-catch attribution to M-REF-ALIGN-UNIT + Mi-17 was correct for the SKIP cluster but missed that the binary-exit-code claim cures inadvertently on any earlier item that adds a passing TEST_CASE to the binary. C-4 surfaced this because its new TEST_CASE landed in `tests/test_reference_mode.cpp` and flipped the binary's exit code before either M-REF-ALIGN-UNIT or Mi-17 dispatched. Cure-attribution sweep rule (`feedback_pre_dispatch_checklist.md`) caught the drift via the C-4 fix-agent's observation; orchestrator gate-evaluated and audit-1 confirmed empirically. Third reinforcing instance of cure-attribution sweep earning rent in the cycle (M-3/`:728`, M-4-FU coverage, this).
