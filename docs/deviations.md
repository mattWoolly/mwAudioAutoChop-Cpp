...
- **Reviewer.** orchestrator (self-recorded after coverage-audit agent surfaced the drift at INT-728 decision time; user-affirmed).
- **Original docs said.** `BACKLOG.md` `INT-728-FIXTURE-MISMATCH` listed three architectural options for the test: (a) fix the arithmetic, (b) rewrite to use the FIXTURE-WAVEEXT artifact (depends on an unfiled `M-3-EMIT`), or (c) drop the TEST_CASE entirely in favor of `test_lossless`'s existing EXTENSIBLE round-trip. `docs/known-failing-tests.md`'s INT-728 entry listed only (a) and (b); option (c) was never propagated.
- **We did.** Reconciled both docs atomically with the close-out: (c) chosen, both docs updated to reflect the same option set and the same decision attribution, with cross-references between them.
- **Reasoning.** Pre-staged governance docs that reference the same item assert claims along **cross-doc-consistency** axes that are not exercised by within-doc audit passes. `KNOWN-FAILING-CURE-ATTRIBUTION-V1` (this file's prior entry) covers within-doc axis coverage — for each cited cure, does the mechanism actually address the failure? `KNOWN-FAILING-VS-BACKLOG-OPTION-DRIFT-V1` covers a different axis: when two docs reference the same item, do they agree on the fact set? In this instance: did `docs/known-failing-tests.md` and `BACKLOG.md` agree on the option set for INT-728? They did not; option (c) was missing from the former. Same gap-family as M-3's cure-attribution catch — specific instance teaches a general rule about governance-doc discipline.
- **Schema decision.** Any decision that touches multiple governance docs (`docs/known-failing-tests.md`, `BACKLOG.md`, `docs/decisions/*.md`, `docs/deviations.md`) gets a **cross-doc reconciliation pass** at decision time, before the close-out commit lands. The pass enumerates every doc that references the item and verifies they agree on (i) option/path set, (ii) cure attribution, (iii) Resolved-vs-Active status, (iv) merging commit. Discrepancies surface as a halt, not a fix-up after the fact.
- **Consequence.** Going forward, every Tier-N close-out paperwork commit is preceded by a cross-doc reconciliation pass. A paperwork commit that updates only one of multiple governance docs referencing the item is a halt signal — the orchestrator pulls the others up and reconciles atomically. Memory entry `feedback_cross_doc_reconciliation.md` records the rule.
- **Meta-note.** This is the **second** time a governance-doc-drift catch has produced a playbook-rule promotion (first: `KNOWN-FAILING-CURE-ATTRIBUTION-V1` on the within-doc axis). Two catches now form a track-record signal: governance-doc drift is a real, recurring failure mode in this remediation cycle, not theoretical. Each catch tightens a different axis (within-doc cure-attribution; cross-doc fact-set consistency). Future governance docs should expect both passes at minimum before being declared authoritative.
...
- **Reasoning.** Pre-staged governance docs assert claims along multiple axes. `docs/known-failing-tests.md` asserts on at least three: (1) test identity (does this TEST_CASE name + tag exist?), (2) failure mode (does it fail at the cited line with the cited assertion?), (3) cure attribution (does the cited cure mechanism actually address the failure?). Prior catches (PR #23 conflation, PR #23 line-shift schema, PR #24 AIFF cluster visibility) covered axes 1+2. Axis 3 was never explicitly swept; the M-3 dispatch caught a long-standing axis-3 error during validation-gate verification. The catch is a discipline-level lesson, not a one-off correction.
- **Schema decision.** Pre-staged docs need audits along **every** claim axis they assert on, not just the most visible one. Auditing surface axes is not transitive evidence about cure-attribution correctness. For each `Cured by: X` claim in `docs/known-failing-tests.md`, the audit reads the cited cure's mechanism and the test's failure mode and reasons whether the mechanism actually addresses the failure. "Same area of code" is not enough; "test fails on file format X, cure changes file format X" is not enough. The semantic chain must connect.
- **Consequence.** Going forward, every Tier 4+ dispatch's pre-flight checks include a cure-attribution sweep on the gate doc against the dispatched item's actual cure mechanism. Any gap surfaces as a halt before dispatch, not as a fix-agent validation failure mid-flight. Memory entry `feedback_audit_pre_staged_docs_along_every_axis.md` records the rule.
- **Meta-note.** This is the **fourth** time `docs/known-failing-tests.md` has been wrong at dispatch/merge time, surfaced by halt-and-surface protocol. Catches 1–3 (PR #23 conflation, PR #23 schema, PR #24 cluster) are documented in `KNOWN-FAILING-SCHEMA-V2` and `KNOWN-FAILING-COMPLETENESS-V1` below. This is the first catch on the cure-attribution axis. Update the count and the "what axes have been audited" tracker accordingly when filing the next governance doc.
